...
| Impact and usefulness | A meaningful problem, highly effective targeted solution, and clear real-world potential | The first users are technical artists and level designers reviewing generated scenes before engine integration. The same verification pattern extends to games, AR/VR, robot simulation, and accessibility review. | Name the first user in the opening sentence. Show the specific failed destination instead of claiming abstract impact. |

## Best of World Labs

| Criterion | Target | WorldSpec evidence |
| --- | --- | --- |
| Integration and creativity, 0-5 | 5 | WorldSpec requires Marble's paired exports. Its Gaussian splat supplies the high-fidelity visual world, its collider supplies testable geometry, and its metric metadata plus persistent world ID keep both representations aligned and refreshable. The test needs both representations. WorldSpec uses Marble as the source of truth for a post-generation verification workflow. |
